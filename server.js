...
app.post('/calculate-tax', (req, res) => {
    let taxMessage = calculateTax(req.body.income, req.body.taxYear, req.body.age, req.body.monthOrYear);
    res.json({ message: taxMessage }); 
});

let sarsTaxYears = [];

if (!fs.existsSync('config/taxConfig.js') || process.argv[2] == 'new-file') setupServer();
...
async function setupServer(){
    await axios.get('https://www.sars.gov.za/tax-rates/income-tax/rates-of-tax-for-individuals/')
        .then(response => {
            ...
            let answer = JSON.stringify(taxObjectsArray) + '\n' + JSON.stringify(taxRebatesArray) + '\n' + JSON.stringify(taxThresholdsArray);
            // res.send(answer);
        })
        .catch(error => {
...
app.get('/tax-years', (req, res) => {
    ...
    sarsTaxYears.length > 0 ? res.json({taxYears: sarsTaxYears, message: 'sarsTaxYears'}) 
